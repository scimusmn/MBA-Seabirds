...
public:
  ...
  void idle(bool act){
    ...
    //if(ave < minRead) cbDone = false;

    if(act){
      if(ave > minRead && !cbDone){
        if(triggerCB && triggerCB(ave)) cbDone = true;
      } else  if(ave < minRead) cbDone = false;
    }
